...
% % Nu_e = 0;
% clear p_n; clear q_n;
% p_n_losses; %этот скрипт подтягивает значения продольных волновых чисел и столбец столкновений из файла disperson_losses_m1.mat, находящегося в этой же директории
...
%     p_n = p_n_los(:,I); %<-- здесь задаем число от 1 до 51 - это будет столбец p_n для соотвтетствующего значения столкновений (для всех столбцов ставим I)
%     q_n = sqrt(1-p_n.^2);
%     q_n = q_n.* (2*(imag(q_n) <= 0)-1);
...
% Зависимость R_alp1 и R_alp2 от q
q = 1e-2:1e-3:0.999;
p = sqrt(1 - q.^2);
