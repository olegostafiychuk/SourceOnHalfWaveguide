function [B1,B2,Cm2, Dm] = coefficientsOfContinuousSpectrum_forPsi2(typeOfCylinder, k_0, k, a_0, EE1, GG1, HH1, MU1, EE, MU, m, p, q, psi, Dm2)
...
%             M33 =  A.* q.* ((p.* (m./a_0)).* (psi.* H1m + H2m));
                %%%% Hphi
             M31 =  A_1.* (-1i * EE1 * k_0 * q1.^2.* dJm1);
             M32 =  A_1.* ((-q1.* p.* m./ a_0).* Jm1);
             M33 =  A.* ( 1i * k_0 * q.^2.* (psi.* dH1m + dH2m));
...
%             R3  =  q.* A.* (1i * MU * k_0 * q.* (-psi.* dH1m + dH2m)); %%%% Ephi
            R3  =  A.* q.* ((p.* (m./a_0)).*(psi.* H1m - H2m)); %%%% Hphi
...
%             M33 =  q.* A.* ((p.* (m./a_0)).* (psi.* H1m + H2m));
            ...
            R2  = q.* (-psi.* H1m + H2m);
%             R3  = q.* A.* (1i * MU * k_0 * q.* (-psi.* dH1m + dH2m)); %%%% Ephi
            R3  =  A.* q.* ((p.* (m./a_0)).*(psi.* H1m - H2m)); %%%% Hphii
...
% mat_boundConditions = [ M11      M12      -R1;
%                         M21      M22      -R2;
%                         M31      M32      -R3];
% %%%% правая (неоднородная) часть системы
% rightSide = [-M13;
